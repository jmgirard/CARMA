...
function [box] = reliability( X )
	k = size(X,2);
	cAlpha = k/(k-1)*(var(nansum(X'))-nansum(var(X)))/var(nansum(X'));
    
    if k == 1
        box = {'# Raters','1'; ...
            '[01] Mean',num2str(nanmean(X),'%.0f'); ...
            '[01] SD',num2str(nanstd(X),'%.0f')};
    elseif k == 2
        box = {'# Raters','2'; ...
            'Alpha',num2str(cAlpha,'%.3f'); ...
            '[01] Mean',num2str(nanmean(X(:,1)),'%.0f'); ...
            '[02] Mean',num2str(nanmean(X(:,2)),'%.0f'); ...
            '[01] SD',num2str(nanstd(X(:,1)),'%.0f'); ...
            '[02] SD',num2str(nanstd(X(:,2)),'%.0f')};
    elseif k > 2
        box = {'# Raters',num2str(k,'%d')};
        box = [box;{'Alpha',num2str(cAlpha,'%.3f')}];
        for i = 1:k
